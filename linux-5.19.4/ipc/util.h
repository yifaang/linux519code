...
#endif
//这个结构体 ipc_params 用于存储在执行 IPC（进程间通信）操作时所需要的参数。结构体包含了不同类型的参数，根据不同的 IPC 操作（如共享内存、信号量等）需要不同的字段
struct ipc_params {
    key_t key;         // IPC 对象的键值，用于标识 IPC 对象。用于区分不同的 IPC 资源。
    
    int flg;           // 标志位，通常用于指定访问权限或其他 IPC 操作的控制选项。
    
    union {
        size_t size;   // 对于共享内存操作（如 shmget），表示共享内存段的大小。
        int nsems;     // 对于信号量操作（如 semget），表示信号量集的大小。
    } u;               // 一个联合体，根据不同的 IPC 类型，选择使用其中的一个字段。
    
    // union 用来根据不同的 IPC 类型存储相应的参数。
    // - 对于共享内存，使用 size 字段指定内存大小。
    // - 对于信号量，使用 nsems 字段指定信号量数量。
};
...
struct ipc_ops {
     ...
     * @brief 用于创建新的 IPC 资源。
     * 
     * @param ipc_namespace 当前 IPC 命名空间，决定资源的作用范围。
     * @param ipc_params 包含资源创建所需的参数，例如键值（key）、大小（size）和标志（flags）。
     * 
     * @return 成功时返回创建的资源 ID（正整数），失败时返回错误码（负值）。
     */
    int (*getnew)(struct ipc_namespace *, struct ipc_params *);

    /**
     * @brief 用于将某个进程与 IPC 资源关联。
     * 
     * @param kern_ipc_perm 内核中描述该资源的权限结构，包含 UID、GID、访问权限等信息。
     * @param shmflg 用户提供的标志位，用于指定访问权限或其他行为。
     * 
    ...
    /**
     * @brief 用于执行额外的权限检查或限制。
     * 
     * @param kern_ipc_perm 当前 IPC 资源的权限结构，描述其所有者和访问权限。
     * @param ipc_params 包含用户请求的参数信息，用于与现有资源匹配。
     * 
     * @return 检查通过返回 0，否则返回错误码。
     */
