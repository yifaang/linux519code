...
/**
 * struct ext4_dir_entry - 用于表示 ext4 文件系统中的目录项。
 * 每个目录项包含文件元数据的基本信息，如 inode 编号和文件名。
 */
struct ext4_dir_entry {
	__le32 inode;             /* Inode 编号。
	                             指向文件系统中此文件的 inode 结构。 */
	__le16 rec_len;           /* 目录项的长度。
	                             表示当前目录项的字节数，包括文件名的存储空间。 */
	__le16 name_len;          /* 文件名的长度（以字节为单位）。
	                             不包括字符串结束符 '\0'。 */
	char name[EXT4_NAME_LEN]; /* 文件名。
	                             文件名的最大长度由 `EXT4_NAME_LEN` 常量定义。 */
};
 ...
 */
/**
 * struct ext4_dir_entry_2 - ext4 文件系统的扩展目录项结构。
 * 相较于 `ext4_dir_entry`，此结构增加了文件类型字段，以便更高效地识别文件类型。
 */
struct ext4_dir_entry_2 {
	...
	__le16 rec_len;           /* 目录项的长度。
	                             表示当前目录项的字节数，包括文件名的存储空间。 */
	__u8 name_len;            /* 文件名的长度（以字节为单位）。
	                             不包括字符串结束符 '\0'。 */
	__u8 file_type;           /* 文件类型。
	                             使用 EXT4_FT_* 宏定义，表明此目录项的文件类型。 */
	char name[EXT4_NAME_LEN]; /* 文件名。
	                             文件名的最大长度由 `EXT4_NAME_LEN` 常量定义。 */
};
