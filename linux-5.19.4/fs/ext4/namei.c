...
struct dx_root
{
	...
	char dot_name[4];                    /* '.' 文件名，通常是 "."  */

	/* 父目录的目录项，即 ".." */
	struct fake_dirent dotdot;           /* '..' 目录项 */
	char dotdot_name[4];                 /* '..' 文件名，通常是 ".." */

	/* 目录索引块的元信息 */
	struct dx_root_info
	{
		__le32 reserved_zero;           /* 保留字段，通常为零 */
		u8 hash_version;                /* 哈希算法版本，指示使用的哈希算法（如 ext4 支持多种哈希） */
		u8 info_length;                 /* 信息长度，通常为 8，表示此结构的大小 */
		u8 indirect_levels;             /* 间接级别数，指示目录索引树的深度 */
		u8 unused_flags;                /* 未使用的标志位 */
	}
	info;

	/* 目录索引条目，包含了实际的目录项。 */
	struct dx_entry entries[];           /* 目录项索引表，实际的目录项数据存储在此 */
};
